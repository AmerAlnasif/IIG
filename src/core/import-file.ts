...
export async function importCodeFile(
  ...
  // whose body matches a new chunk's, reuse the existing embedding.
  // Only truly new/changed chunks hit the
  // OpenAI API. The match runs on the header-stripped body: the header
  // carries line numbers and the index shifts when a symbol is added above,
  // so keying on either re-embedded byte-identical bodies.
  ...
  const existingChunks = existing ? await engine.getChunks(slug, { ...(sourceId ? { sourceId } : {}), includeEmbedding: true }) : [];
